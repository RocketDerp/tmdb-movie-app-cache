...
{#if shown}
	<section id="trailer">
		<div class="fixed top-0 bottom-0 left-0 right-0 z-50 bg-gray-800 bg-opacity-80">
			<div
				class="absolute inset-0 m-auto h-[50.625vw] max-h-[90vh] w-[90vw] max-w-[160vh] border-8 border-teal-600"
			>
				{#if video_site === 'YouTube'}
					<iframe
						class="iframe absolute top-0 left-0 h-full w-full bg-contain"
						title="Trailer"
			...
			<div>
				<button class="absolute left-3 top-3 " aria-label="close" on:click={() => hide()}>
					<svg
